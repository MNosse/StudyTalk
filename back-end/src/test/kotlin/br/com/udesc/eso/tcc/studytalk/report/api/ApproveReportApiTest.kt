...
class ApproveReportApiTest @Autowired constructor(
    ...
    fun setup(testInfo: TestInfo) {
        RestAssured.baseURI = "http://localhost:$port/studytalk/api/reports/approve"
    ...
    fun withValidValues1() {
        RestAssured.given()
            .pathParam("id", 1L)
            .param("approverUid", participant2Uid)
            .`when`()
            .delete("/{id}/")
            .then()
    ...
    fun withInvalidApproverUid() {
        RestAssured.given()
            .pathParam("id", 1L)
            .param("approverUid", participant2Uid + "a")
            .`when`()
            .delete("/{id}/")
            .then()
            .statusCode(HttpStatus.NOT_FOUND.value())
